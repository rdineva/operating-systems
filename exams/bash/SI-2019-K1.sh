...
find $(cat /etc/passwd | cut -d: -f6) -maxdepth 0 -type d -exec stat --printf="%n %Y\n" {} \; 2> /dev/null | awk '$2 > 1582898637 && $2 < 1631190775 {print $1}' | xargs -I % sh -c 'cat /etc/passwd | cut -d: -f1,5,6 | egrep "^.*:.*:%$"' | cut -d: -f1,2 | tr : '\t'
