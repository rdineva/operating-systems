...
# Зад. 5 2017-IN-03
# Напишете серия от команди, които от файла /etc/passwd да вземат под-низ,
# състоящ се от втора и трета цифра на факултетния номер на студентите от
# специалност Информатика,чиито фамилии завършват на “а”. Изведете коя комбинация
# от цифри се среща най-често и коя е тя.
...
egrep '.*, Inf, .*' passwd | egrep ".*:.*:.*:.*a, .*" | cut -d : -f 1 | tr -d 's' | cut -c 2,3 | sort -n | uniq -c | head -n 1
